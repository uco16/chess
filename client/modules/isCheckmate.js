...
export default function isCheckmate(position, playerColor, enPassantTarget, canCastle, activeColor) {
  ...
  for (let piecePos of playerColorPiecePositions) {
    ...
    for (let endPos of accessibleSquares(piecePos, pieceType, pieceColor)) {
      if (isLegal(piecePos, endPos, position, enPassantTarget, playerColor, canCastle, activeColor)) {
